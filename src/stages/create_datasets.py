...
def split_data(data_lines: list[dict], train_fracture: float, val_fracture: float, test_fracture: float, random_seed: int)->tuple[list[dict], list[dict], list[dict]]:
    train_count = int(train_fracture * len(data_lines) / (train_fracture + val_fracture + test_fracture))
    val_count = int(val_fracture * len(data_lines) / (train_fracture + val_fracture + test_fracture))
    test_count = int(test_fracture * len(data_lines) / (train_fracture + val_fracture + test_fracture))
...
def build_all_masks(annotations: list[dict], target_categories: list[int], width: int, height: int) -> np.ndarray:
...
def create_dataset_from_annotation_lines(image_data_lines: list[dict], image_dataset_filename: str, masks_dataset_filename: str, target_categories: list[int], scaling_sequence):
    folder = tempfile.mkdtemp()
    try:
        urls = [item['url'] for item in image_data_lines]
        image_ids = [item['id'] for item in image_data_lines]
        image_filenames = [ generate_image_name(image_url, image_id) for image_url, image_id in zip(urls, image_ids)  ]
                ...
                for line in tqdm.tqdm(image_data_lines):
                    ...
                    try:
                        ...
                        if image is None:
                            ...
                            continue
                        masks = build_all_masks(line["annotations"], target_categories, line['width'], line['height'] )
                        transformed = scaling_sequence(image=image, mask=masks)

                        image_file.create_dataset(image_id, data=transformed['image'], dtype='uint8', compression='gzip')
                        mask_file.create_dataset(image_id, data=transformed['mask'], dtype='uint8', compression='gzip')
                        mask_file[image_id].attrs['mask_points'] = transformed['mask'].sum()
...
if __name__ == "__main__":
    ...
    from src.core.image_processing import create_scaling_sequence, create_scaling_transform
    ...
    with open( os.path.join(project_root, config.dataset.raw_files_folder, "parsed_annotations.json"), "r" ) as input_file:
        full_data_lines = json.load(input_file)
        
    train_lines, val_lines, test_lines = split_data(full_data_lines, config.dataset.train_fracture, config.dataset.val_fracture, config.dataset.test_fracture, config.random_seed)
    ...
    scaling_sequence = create_scaling_transform(config.dataset.image_size, config.random_seed)


    print("Started val images download")
    create_dataset_from_annotation_lines(val_lines, os.path.join(datasets_folder, "val_images.h5py"), os.path.join(datasets_folder, "val_masks.h5py"), target_categories, scaling_sequence)
    print("Started test images download")
    create_dataset_from_annotation_lines(test_lines, os.path.join(datasets_folder, "test_images.h5py"), os.path.join(datasets_folder, "test_masks.h5py"), target_categories, scaling_sequence)
    print("Started train images download")
    create_dataset_from_annotation_lines(train_lines, os.path.join(datasets_folder, "train_images.h5py"), os.path.join(datasets_folder, "train_masks.h5py"), target_categories, scaling_sequence)
