...
def extract_needed_annotations( annotations_lines: list[dict], target_categories: list[int] ) -> dict[list[dict]] :
    parsed_annotations = dict()
    for item in annotations_lines:
        if item["category_id"] not in target_categories or "segmentation" not in item.keys():
            continue
        annotations_list = parsed_annotations.get( item["image_id"], [] )
        for polygon in item["segmentation"]:
            annotations_list.append( { "annotation": polygon, "category" : item["category_id"]} )
        parsed_annotations[item["image_id"]] = annotations_list
    return parsed_annotations

def filter_images(image_data_lines: list[dict], parsed_annotations: dict[list[dict]]) -> list[dict]:
    images_data = []
...
def parse_annotations_file(input_file_path: str, target_categories: list[int]) -> list[dict]:
...
if __name__ == "__main__":
    ...
    parsed_train_annotations = parse_annotations_file(os.path.join(raw_files_folder, "train_anno.json"), config.dataset.coco_categories_map.keys())
    parsed_val_annotations = parse_annotations_file(os.path.join(raw_files_folder, "val_anno.json"), config.dataset.coco_categories_map.keys())
    output = parsed_train_annotations + parsed_val_annotations

    with open( os.path.join(raw_files_folder, "parsed_annotations.json"), "w" ) as output_file:
