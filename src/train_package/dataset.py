...
class SegmentationDataset(Dataset):
    def __init__(self, images_file_path: str, masks_file_path: str, feature_extractor, transform: A.DualTransform, additional_segmentation_transform: A.DualTransform=None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        if self.additional_segmentation_transform is not None:
            transformed['mask'] = self.additional_segmentation_transform(image = transformed['image'], mask=transformed['mask'])['mask']
